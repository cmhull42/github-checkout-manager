...
class DeploymentManager:
    def __init__(self):
        ...
        with open("config.json", "r") as f:
            self.repoconfig = json.loads(f.readlines())
    ...
    def receivepush(self, pushdata):
        ...
        subprocess.run("git reset --hard origin/master".split(), cwd=foundrepo["repodirectory"], env=myenv)
        subprocess.run("git pull origin master".split(), cwd=foundrepo["repodirectory"], env=myenv)

        copy_tree(os.path.expanduser(foundrepo["repodirectory"]), os.path.expanduser(foundrepo["contentdirectory"]))